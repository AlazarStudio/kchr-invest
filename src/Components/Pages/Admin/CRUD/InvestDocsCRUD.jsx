...
export const InvestDocsList = props => (
	<List {...props}>
		<Datagrid>
			<TextField source='InvestGroup.title' label='Группа' />
			<TextField source='title' label='Название' />
		</Datagrid>
	</List>
)

export const InvestDocsEdit = props => {
	...
	const handleAddGroup = async () => {
		...
		if (title) {
			try {
				await dataProvider.create('invest-group', { data: { title } })
...
export const InvestDocsCreate = props => {
	const refresh = useRefresh()
	const notify = useNotify()
	const dataProvider = useDataProvider()
	...
	return (
		<Create {...props} transform={handleSaveDocument}>
			<SimpleForm>
				<ReferenceInput source='groupId' reference='invest-group'>
					<SelectInput optionText='title' required label='Группа' />
				</ReferenceInput>
				<Button label='Добавить группу' onClick={handleAddGroup}>
					<AddIcon />
				</Button>
